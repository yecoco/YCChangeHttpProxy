...
- (void)setProxy:(NSString *)ipaddr port:(NSUInteger)port {
		...
		//SCPreferencesRef prefRef = SCPreferencesCreateWithAuthorization(NULL, CFSTR("test_proxy"), NULL, NULL);

		if(!SCPreferencesLock(prefRef, true))
		{
			DDLog(@"SCPreferencesLock failed");
		}
	   	...
	   	if (currentSet) {
		    ...
		    if (wifiServiceKey) {
			    ...
			    [proxies setObject:@(1) forKey:cfs2nss(kSCPropNetProxiesHTTPEnable)];
			   	...
			   	DDLog(@"end proxies = %@",proxies);
			    if(!SCPreferencesSetValue(prefRef, kSCPrefNetworkServices, (__bridge CFPropertyListRef)nservices))
			    {
			    	DDLog(@"SCPreferencesSetValue failed");
			    }
			    if (!SCPreferencesCommitChanges(prefRef))
			    {
			    	DDLog(@"SCPreferencesCommitChanges failed");
			    }
			    if (!SCPreferencesApplyChanges(prefRef))
			    {
			    	DDLog(@"SCPreferencesApplyChanges failed");
			    }
		    } else {
